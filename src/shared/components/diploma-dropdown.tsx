...
export default function DiplomaDropDown<
...
>({
  ...
  const { data, isLoading, isFetchingNextPage, fetchNextPage, hasNextPage } =
    useGetPaginatedDiplomas({ limit, search: debouncedSearch });
    ...
    <Field>
      ...
      {control ? (
        ...
        <p className="text-destructive font-mono text-xs">
          Error: Diplomas must be used within a FormProvider or passed a control
          prop.
        </p>
      )}
...
> {
  ...
  disabled: boolean;
  isLoading: boolean;
  diplomas: Array<{ id: string; title: string }>;
  hasNextPage?: boolean;
...
function ControllerWrapper<TFieldValues extends FieldValues = FieldValues>({
  ...
  return (
    <Controller
      ...
      control={control}
      render={({ field }) => {
        const selectedExam = diplomas.find((exam) => exam.id === field.value);
